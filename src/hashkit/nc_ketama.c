...
ketama_update(struct server_pool *pool)
{
    ...
    for (server_index = 0; server_index < nserver; server_index++) {
        ...
        for (pointer_index = 1;
            ...
            for (x = 0; x < pointer_per_hash; x++) {
                ...
                sprintf(pool->continuum[continuum_index].name, server->pname.data);
                pool->continuum[continuum_index++].value = value;
                

                //log_warn("redis服务器在一致性哈希中的信息 %.*s, key:%s, value:%ld, redis sever序号:%ld", server->pname.len, server->pname.data, host, value, server_index); 
            }
        }
    ...
    struct continuum *begin;
    
    for(begin =pool->continuum; begin<pool->continuum + pool->ncontinuum;begin++) {
        //log_warn("服务器在一致性哈希中的值 %s %d %ld",begin->name, begin->index, begin->value);
        log_debug("redis服务器在一致性哈希中的信息 %s, value:%ld, redis sever序号:%ld", begin->name, begin->value, begin->index); 
    }
