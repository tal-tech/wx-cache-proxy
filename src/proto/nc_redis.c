...
redis_arg0(struct msg *r)
{
    ...
    case MSG_REQ_REDIS_SELECT_CLIENT:
    case MSG_REQ_REDIS_SET_SLOW_THRESHOLD:
        return true;
        ...
        case SW_REQ_TYPE:
            ...
            case 6:
                ...
                if (str6icmp(m, 's', 'e', 'l', 'e', 'c', 't')) {
                    r->type = MSG_REQ_REDIS_SELECT_CLIENT;
                    r->noforward = 1;
                    break;
                }
            ...
            case 9:
                ...
                if (str9icmp(m, 't', 'h', 'r', 'e', 's', 'h', 'o', 'l', 'd')) {
                    r->type = MSG_REQ_REDIS_SET_SLOW_THRESHOLD;
                    r->noforward = 1;
                    break;
                }
...
redis_reply(struct msg *r)
{
    ...
    if (r->type == MSG_REQ_REDIS_SET_SLOW_THRESHOLD) {
        return redis_handle_threshold_req(r, response);
    }
...
int g_slow_threshold;
rstatus_t
redis_handle_threshold_req(struct msg *req, struct msg *rsp)
{
    struct conn *conn = (struct conn *)rsp->owner;
    struct server_pool *pool;
    struct keypos *kpos;
    ...
    int before = g_slow_threshold;
    g_slow_threshold = atoi(key);

    log_warn("pid %d, change threshold from %d to %d", getpid(), before, g_slow_threshold);
    /*
     * Password in the AUTH command doesn't match the one configured in
     * redis_auth: directive
     *
