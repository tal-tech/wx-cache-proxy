...
void _nc_free(void *ptr, const char *name, int line) {
    ASSERT(ptr != NULL);log_debug(LOG_VVERB, "free(%p) @ %s:%d", ptr, name, line);
    free(ptr);
}
...
static int nc_resolve_inet(struct string *name, int port, struct sockinfo *si) {
    int status;
    ...
    if (status != 0) {
        log_error(
                "address resolution of node '%s' service '%s' failed: %s", node, service, gai_strerror(status));
        return -1;
    }
...
static int nc_resolve_unix(struct string *name, struct sockinfo *si) {
