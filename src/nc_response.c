...
rsp_forward(struct context *ctx, struct conn *s_conn, struct msg *msg)
{
    ...
    log_debug(LOG_NOTICE, "recv_server_time %ld\n", msg->recv_server_time);
...
//    log_warn("HHHHHHHH send_client_time %s", send_client_time->str);
//    pmsg->send_client_time = send_client_time->str;
...
//    printf("ffffffffffffffffffff %s \n", mbuf->start);
    ...
    conn->dequeue_outq(ctx, conn, pmsg);
    pmsg->send_client_time = get_current_time();
