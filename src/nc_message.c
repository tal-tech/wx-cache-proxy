...
msg_put(struct msg *msg)
{
    ...
    if (msg->keys) {
        ...
        msg->keys = NULL;
        msg->recv_client_time = 0;
        msg->send_server_time = 0;
        msg->recv_server_time = 0;
        msg->send_client_time = 0;
    }
...
//            GString *recv_server_time = g_string_sized_new(sizeof("2004-01-01T00:00:00.000Z"));
//            get_current_time_str(recv_server_time);
//            msg->recv_server_time = recv_server_time->str;

//            msg->recv_server_time = get_current_time();
//            log_warn("aaaaaaaaaaaaaaaaaaaaaaaaaaa recv_server_time %ld\n", msg->recv_server_time);
        }
...
//        get_current_time_str(recv_server_time);
//        msg->recv_server_time = recv_server_time->str;
//        log_warn("  kkkkkkkkkkkkkkkkkkkkkkkkkk recv_server_time %s", recv_server_time->str);
    }
    else 
    {
        //说明此时角色为client
        //zhangming 设置接收时间
//        GString *recv_client_time = g_string_sized_new(sizeof("2004-01-01T00:00:00.000Z"));
//        get_current_time_str(recv_client_time);
//        msg->recv_client_time = recv_client_time->str;
//        log_warn("AAAAAAAAAAAA recv_client_time %s", recv_client_time->str);

        msg->recv_client_time = get_current_time();
        log_debug("recv_client_time %ld", msg->recv_client_time);
    }
    ...
    for (msg = TAILQ_FIRST(&send_msgq); msg != NULL; msg = nmsg) {
        ...
        if (mbuf == NULL) {
            ...
            if(!conn->client && !conn->proxy)
            {
                //说明此时角色为server
                //zhangming 设置接收时间
//                GString *send_server_time = g_string_sized_new(sizeof("2004-01-01T00:00:00.000Z"));
//                get_current_time_str(send_server_time);
//                msg->send_server_time = send_server_time->str;
//                log_warn("AAAAAAAAAAAAAAA send_server_time %s", send_server_time->str);

                msg->send_server_time = get_current_time();
            }
            else 
            {
                //说明此时角色为client
                //zhangming 设置接收时间
//                GString *send_client_time = g_string_sized_new(sizeof("2004-01-01T00:00:00.000Z"));
//                get_current_time_str(send_client_time);
//                log_warn("AAAAAAAAAAAAAAA send_client_time %s", send_client_time->str);
//                msg->send_client_time = send_client_time->str;
            }  
