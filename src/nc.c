...
#define NC_CONF_PATH        "conf/proxy.yml"
...
#define NC_ETCD_CONF_PATH         NULL
#define NC_MQ_CONF_PATH         NULL
#define NC_TRACE_CONF_PATH         NULL
...
static struct option long_options[] = {
    ...
    { "etcd-conf",      required_argument,  NULL,   'e' },
    { "mq-conf",      required_argument,  NULL,   'M' },
    { "trace-log",      required_argument,  NULL,   'L' },
    { NULL,             0,                  NULL,    0  }
};

static char short_options[] = "hVtdDv:o:c:s:i:a:p:m:n:e:M:N:T:L:";
...
{
    log_stderr(
        "Usage: wx-cache-proxy [-?hVdDt] [-v verbosity level] [-o output file]" CRLF
        "                  [-c conf file] [-s stats port] [-a stats addr]" CRLF
        "                  [-i stats interval] [-p pid file] [-m mbuf size]" CRLF
        "                  [-n worker process number]" CRLF
        "                  [-e etcd conf file]" CRLF
        "                  [-L trace log]" CRLF
        "");
    ...
    log_stderr(
        ...
        "  -n, --worker-num=N     : set worker process number (default: %d)" CRLF
        "  -e, --etcd-conf=S     : set etcd conf filename (default: %s)" CRLF
        "  -L, --trace-conf=S     : trace req/resp conf (default: %s) " CRLF
        "",
...
nc_set_default_options(struct instance *nci)
{
    ...
    nci->etcd_conf_filename = NC_ETCD_CONF_PATH;
    ...
    nci->pidfile = 0;
    core_worker_num = 1;
    nci->mq_consume_worker_num = 0;
    nci->mq_conf_filename = NC_MQ_CONF_PATH;
    nci->environment_type = 0;
    nci->trace_conf_filename = NC_TRACE_CONF_PATH;
}
...
nc_get_options(int argc, char **argv, struct instance *nci)
{
    ...
    for (;;) {
        ...
        case 'n':
        	core_worker_num = atoi(optarg);
        	if (core_worker_num > 128) {
        		log_stderr("wx-cache-proxy: option -n is too big");
        		return NC_ERROR;
        	}
            break;
        ...
        case 'v':
            ...
            if (value < 0) {
                log_stderr("wx-cache-proxy: option -v requires a number");
                return NC_ERROR;
            }
            nci->log_level = value;
            break;
        ...
        case 's':
            value = nc_atoi(optarg, strlen(optarg));
            if (value < 0) {
                log_stderr("wx-cache-proxy: option -s requires a number");
                return NC_ERROR;
            }
            if (!nc_valid_port(value)) {
                log_stderr("wx-cache-proxy: option -s value %d is not a valid "
                           "port", value);
                return NC_ERROR;
            }
        ...
        case 'i':
            value = nc_atoi(optarg, strlen(optarg));
            if (value < 0) {
                log_stderr("wx-cache-proxy: option -i requires a number");
                return NC_ERROR;
            }
        ...
        case 'm':
            value = nc_atoi(optarg, strlen(optarg));
            if (value <= 0) {
                log_stderr("wx-cache-proxy: option -m requires a non-zero number");
                return NC_ERROR;
            }

            if (value < NC_MBUF_MIN_SIZE || value > NC_MBUF_MAX_SIZE) {
                log_stderr("wx-cache-proxy: mbuf chunk size must be between %zu and"
        ...
        case 'e':
        	nci->etcd_conf_filename = optarg;
            break;
        
        case 'M':
        	nci->mq_conf_filename = optarg;
            break;
        
        case 'N':
        	nci->mq_consume_worker_num = atoi(optarg);
        	if (nci->mq_consume_worker_num > 24) {
        		log_stderr("wx-cache-proxy: option -N is too big");
        		return NC_ERROR;
        	}
            break;
        ...
        case 'T':
        	nci->environment_type = atoi(optarg);
        	if (nci->environment_type > 2) {
        		log_stderr("wx-cache-proxy: option -T is too big");
        		return NC_ERROR;
        	}
            break;
        
        case 'L':
        	nci->trace_conf_filename = optarg;
            break;
        ...
        case '?':
            ...
            case 'M':
            case 'C':
                log_stderr("wx-cache-proxy: option -%c requires a file name",
                           optopt);
                break;
            ...
            case 'i':
            case 'n':
                log_stderr("wx-cache-proxy: option -%c requires a number", optopt);
                break;

            case 'a':
                log_stderr("wx-cache-proxy: option -%c requires a string", optopt);
                break;

            default:
                log_stderr("wx-cache-proxy: invalid option -- '%c'", optopt);
                break;
            }
            return NC_ERROR;
...
nc_test_conf(struct instance *nci)
{
    ...
    if (cf == NULL) {
        log_stderr("wx-cache-proxy: configuration file '%s' syntax is invalid",
                   nci->conf_filename);
        return false;
    }
    ...
    log_stderr("wx-cache-proxy: configuration file '%s' syntax is ok",
               nci->conf_filename);
    return true;
}
...
main(int argc, char **argv)
{
    ...
    if (show_version) {
        log_stderr("This is wx-cache-proxy-%s" CRLF, NC_VERSION_STRING);
        if (show_help) {
            nc_show_usage();
        }
