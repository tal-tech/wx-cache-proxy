...
server_pool_idx(struct server_pool *pool, uint8_t *key, uint32_t keylen)
{
    ...
    case DIST_KETAMA:
        ...
        log_debug("server_pool_hash key为%.*s, 哈希值为%ld, pool->ncontinuum %d, idx %d", keylen, key, hash, pool->ncontinuum, idx);
        break;
...
server_pool_server(struct server_pool *pool, uint8_t *key, uint32_t keylen)
{
    ...
    log_debug("%.*s 定位到 %s:%d %.*s", keylen,key, server->addrstr.data, server->port, server->name.len, server->name.data);
